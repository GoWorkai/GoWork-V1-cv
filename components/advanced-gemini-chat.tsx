...
export function AdvancedGeminiChat({ onClose, isMinimized, onToggleMinimize }: AdvancedGeminiChatProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      content:
        "¡Hola! Soy Gow 😊 Estoy aquí para ayudarte en GoWork. ¿Buscas servicios o quieres ofrecer tus talentos?",
      sender: "ai",
      timestamp: new Date(),
      type: "text",
    },
  ...
  const generateContextualResponse = async (userMessage: string, context: ExtractedData): Promise<string> => {
    const contextPrompt = `
Eres Gow, asistente IA de GoWork. Responde de forma breve, precisa y gentil.

Contexto del usuario:
- Tipo: ${context.userType}
- Habilidades: ${context.professionalInfo.skills.join(", ")}
...
2. Tono gentil y colaborativo
3. 1 pregunta específica para ayudar mejor
4. Solución práctica si aplica
5. Máximo 1 emoji

Responde en español chileno, máximo 60 palabras.
`
